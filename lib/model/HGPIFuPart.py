...
class HGPIFuPart(BasePIFuNet):
    ...
    def query(self, points, calibs, transforms=None, labels=None, parts=None):
        ...
        for i, im_feat in enumerate(self.im_feat_list):
            ...
            pred = in_bb * pred
            out_part = part[:, :-1, :].clone() * in_bb
            out_part[:, -1:, :] = torch.logical_not(in_bb)
    ...
    def forward(self, images, points, calibs, labels, parts, transforms=None):
        self.gt_parts = parts
        self.labels = labels
        self.class_weight = class_weight.compute_class_weight('balanced',np.unique(parts), parts.detach().cpu().numpy())
