...
class HGPIFuPTF(BasePIFuNet):
    ...
    def query(self, points, calibs, transforms=None, labels=None, parts=None):
        ...
        for i, im_feat in enumerate(self.im_feat_list):
            ...
            self.intermediate_corr_list.append(corr)

        self.parts = self.intermediate_parts_list[-1]
        self.preds = self.intermediate_preds_list[-1]
        self.correspondences = self.intermediate_corr_list[-1]

    def get_error(self):
        ## TODO: Add Error combination
        pass
    ...
    def forward(self, images, points, calibs, labels, parts, correspondences, transforms=None):
        self.gt_parts = parts.argmax(1)
        self.labels = labels
        self.gt_correspondences = correspondences

        self.filter(images)

        self.query(points=points, calibs=calibs, transforms=transforms, labels=labels, parts=parts, correspondences=correspondences)
